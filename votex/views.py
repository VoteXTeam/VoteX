...
from votex.api_helpers import user_dependent_call, user_election_dependent_call, election_dependent_call
from votex.controllers import VoterController, ElectionController
from votex.models import Voter, Location, Election
from votex.serializers import LoginUserInputSerializer, VoterSerializer, MessageSerializer, UserIDSerializer, \
...
class StatisticAPI(APIView):
    ...
    @user_dependent_call
    def get(self, request, voter: Voter):
        controller = VoterController(voter=voter)
        elections = controller.get_elections_by_voter()
        serializer = StatisticSerializer(elections, many=True)
